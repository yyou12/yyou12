...
var _ = g.Describe("[sig-cluster-lifecycle] Cluster_Infrastructure", func() {
	...
	g.It("Author:huliu-Low-45343-[MHC] - nodeStartupTimeout in MachineHealthCheck should revert back to default [Flaky]", func() {
		...
		err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("mhc/machine-api-termination-handler", "-n", machineAPINamespace, "-p", `{"spec":{"nodeStartupTimeout":"30m"}}`, "--type=merge").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		nodeStartupTimeoutAfterUpdate, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("mhc", "machine-api-termination-handler", "-o=jsonpath={.spec.nodeStartupTimeout}", "-n", machineAPINamespace).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
