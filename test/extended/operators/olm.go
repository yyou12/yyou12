...
var _ = g.Describe("[sig-operators] OLM on VM for an end user handle within a namespace", func() {
	...
	g.It("VMonly-ConnectedOnly-Author:kuiwang-Medium-30835-complete operator upgrades automatically based on SemVer setting default channel in opm alpha bundle build", func() {
		var (
			...
			bundleImageTag1 = "quay.io/olmqe/cockroachdb-operator:2.0.9-30835"
			bundleImageTag2 = "quay.io/olmqe/cockroachdb-operator:2.1.11-30835"

			// these index are generated by case, need to ensure to remove them after case exits.
			indexImageTag1 = "quay.io/olmqe/cockroachdb-index:2.0.9-30835-" + getRandomString()
			indexImageTag2 = "quay.io/olmqe/cockroachdb-index:2.1.11-30835-" + getRandomString()
		...
		g.By("install operator")
		sub.createWithoutCheck(oc, itName, dr)
		sub.expectCSV(oc, itName, dr, "cockroachdb.v2.1.11")
	...
	g.It("VMonly-ConnectedOnly-Author:kuiwang-Medium-30860-complete operator upgrades automatically based on SemVer instead of replaces or skips [Slow]", func() {
		var (
			...
			// these bundles are prepared data, do not need to remove them after case exits.
			bundleImageTag1 = "quay.io/olmqe/cockroachdb-operator:2.0.9-30860"
			bundleImageTag2 = "quay.io/olmqe/cockroachdb-operator:2.1.11-30860"
			bundleImageTag3 = "quay.io/olmqe/cockroachdb-operator:2.1.1-30860"

			// these index are generated by case, need to ensure to remove them after case exits.
			indexImageTag1 = "quay.io/olmqe/cockroachdb-index:2.0.9-30860-" + getRandomString()
			indexImageTag2 = "quay.io/olmqe/cockroachdb-index:2.1.11-30860-" + getRandomString()
			indexImageTag3 = "quay.io/olmqe/cockroachdb-index:2.1.1-30860-" + getRandomString()
		...
		if err != nil {
			...
			if strings.Compare(state, "UpgradeAvailable") == 0 {
				newCheck("expect", asAdmin, withoutNamespace, compare, "cockroachdb.v2.1.1", ok, []string{"sub", sub.subName, "-n", sub.namespace, "-o=jsonpath={.status.installedCSV}"}).check(oc)
			} else {
	...
	g.It("VMonly-ConnectedOnly-Author:kuiwang-Medium-30674-complete operator upgrades automatically based on SemVer without setting default channel", func() {
		var (
			...
			// these bundles are prepared data, do not need to remove them after case exits.
			bundleImageTag1 = "quay.io/olmqe/cockroachdb-operator:2.0.9-30674"
			bundleImageTag2 = "quay.io/olmqe/cockroachdb-operator:2.1.11-30674"

			// these index are generated by case, need to ensure to remove them after case exits.
			indexImageTag1 = "quay.io/olmqe/cockroachdb-index:2.0.9-30674-" + getRandomString()
			indexImageTag2 = "quay.io/olmqe/cockroachdb-index:2.1.11-30674-" + getRandomString()
	...
	g.It("VMonly-ConnectedOnly-Author:kuiwang-Medium-29810-The bundle and index image reated successfully when spec replaces field is null", func() {
		var (
			...
			bundleImageTag1 = "quay.io/olmqe/cockroachdb-operator:2.0.9-29810-" + getRandomString()
			bundleImageTag2 = "quay.io/olmqe/cockroachdb-operator:2.1.11-29810-" + getRandomString()

			// these index are generated by case, need to ensure to remove them after case exits.
			indexImageTag1 = "quay.io/olmqe/cockroachdb-index:2.0.9-29810-" + getRandomString()
			indexImageTag2 = "quay.io/olmqe/cockroachdb-index:2.1.11-29810-" + getRandomString()
		...
		if output, err = opmCLI.Run("alpha").Args("bundle", "build", "-d", "2.0.9", "-b", "podman", "-t", bundleImageTag1, "-p", "cockroachdb", "-c", "alpha", "-e", "alpha").Output(); err != nil {
			e2e.Logf(output)
			o.Expect(err).NotTo(o.HaveOccurred())
		}
		...
		if output, err = opmCLI.Run("alpha").Args("bundle", "build", "-d", "2.1.11", "-b", "podman", "-t", bundleImageTag2, "-p", "cockroachdb", "-c", "alpha", "-e", "alpha").Output(); err != nil {
			e2e.Logf(output)
			o.Expect(err).NotTo(o.HaveOccurred())
		}
		if !strings.Contains(output, "Writing annotations.yaml") || !strings.Contains(output, "Writing bundle.Dockerfile") {
